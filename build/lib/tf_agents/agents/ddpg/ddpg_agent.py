...
class DdpgAgent(tf_agent.TFAgent):
  ...
  def _train(self, experience, weights=None):
    ...
    with tf.GradientTape(watch_accessed_variables=False) as tape:
      ...
      actor_loss = self.actor_loss(time_steps, next_time_steps, weights=weights, training=True)
  ...
  def _loss(self, experience, weights=None, training=False):
    ...
    actor_loss = self.actor_loss(time_steps, next_time_steps, weights=weights, training=training)
    tf.debugging.check_numerics(actor_loss, 'Actor loss is inf or nan.')
